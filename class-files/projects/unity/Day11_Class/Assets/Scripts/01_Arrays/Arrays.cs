...
using Debug = ClassUtilities.Debug;
...
public class Arrays : MonoBehaviour {
	...
	private float[] SunshineHours = {3f, 3f, 6f, 7f, 9f, 10f, 12f, 11f, 9f, 7f, 6f, 3f};
	...
	void Start () {
		...
		//    black to yellow)
		for (int i = 0; i < SunshineHours.Length; i += 1) {
			Vector3 pos = new Vector3(i * 2f, 0f, 0f);
			GameObject clone = (GameObject) Instantiate(SimpleCube, pos, 
				Quaternion.identity, transform);
			clone.transform.localScale = new Vector3(1f, SunshineHours[i], 1f);
			Material mat = clone.GetComponent<MeshRenderer>().material;
			mat.color = new Color(SunshineHours[i] / 12f, SunshineHours[i] / 12f, 0f);
		}
